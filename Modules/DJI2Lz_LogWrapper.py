...
class DJILogWrapper:
    ...
    def simplify_csv(self, csv_path):
        try:
            ...
            output_path = f"{base_path}_HUDdata.csv"
    ...
    def process_log(self, input_file, output=None, images=None, thumbnails=None,
                   geojson=None, kml=None, csv=None, raw=False, simplify=False):
        self._check_binary()
        ...
        cmd = [self.binary_name, '--api-key', self.api_key]
        
        if output:
            cmd.extend(['-o', str(base_dir / output)])
        if images:
            cmd.extend(['-i', str(base_dir / images)])
        if thumbnails:
            cmd.extend(['-t', str(base_dir / thumbnails)])
        if geojson:
            cmd.extend(['-g', str(base_dir / geojson)])
        if kml:
            cmd.extend(['-k', str(base_dir / kml)])
        if csv:
            csv_path = str(base_dir / csv)
        ...
        try:
            ...
            if csv and simplify:
                simplified_csv = self.simplify_csv(csv_path)
...
def main():
    ...
    if output_type == 'csv':
        input_path = Path(input_file)
        output_path = f"{input_path.stem}.csv"
